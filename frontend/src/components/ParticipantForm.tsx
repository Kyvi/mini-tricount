...
import { ApiHttpError, createParticipant } from '../api/groups';
...
export function ParticipantForm({ groupId, onCreated, onCancel }: ParticipantFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const participant = await createParticipant(groupId, { name: name.trim() });
    ...
    <form className="form-card" onSubmit={handleSubmit}>
      ...
      <div className="field">
        ...
        <input
          ...
          aria-describedby={
            nameError || fieldErrors.name ? 'participant-name-error' : undefined
          }
        />
        {(nameError || fieldErrors.name) && (
          <p id="participant-name-error" className="field-error">
            {nameError ?? fieldErrors.name}
          </p>
        )}
      </div>
